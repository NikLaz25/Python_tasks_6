...
# print(f'Масса асфальта {result_obj.mas} тонн') # ???Вопрос!!! Непонятно почему здесь не работает print

'''Долго разбирался как сделать, чтобы метод работал и с глобальными 
и с локальными переменными. В данной задаче это реализовано'''
